...
U8G2_SSD1327_WS_128X128_F_4W_HW_SPI u8g2(U8G2_R0, /* cs=*/ 7, /* dc=*/ 8, /* reset=*/ -1);
...
static const unsigned char initmsg[]          PROGMEM = "SG.Basic " kVersion;
...
void setup()
{
  ...
  Wire.begin();
  u8g2.begin();  
  u8g2.setFont(u8g2_font_t0_11_tr);  // set the font for the terminal window
  u8g2.setDisplayRotation(U8G2_R3);  // rotates the display 270 degrees
  u8g2log.begin(u8g2, U8LOG_WIDTH, U8LOG_HEIGHT, u8log_buffer);
  u8g2log.setLineHeightOffset(0); // set extra space between lines in pixel, this can be negative
  u8g2log.setRedrawMode(1);   // 0: Update screen with newline, 1: Update screen for every char
  // while( !Serial ); // for Leonardo
  //u8g2log.println ( sentinel );
...
void cmd_Files( void )
{
  ...
  while( true ) {
    ...
    if( entry.isDirectory() ) {
      // directory ending
      //i<16
      for( int i=strlen( entry.name()) ; i<10 ; i++ ) {
      ...
      // file ending
      //i<17
      for( int i=strlen( entry.name()) ; i<13 ; i++ ) {
        printmsgNoNL( spacemsg );
      }
